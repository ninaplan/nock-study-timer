...
export default function HomeTab({ t, creds, settings, isDemoMode }) {
  ...
  const onTouchEnd   = (e) => {
    ...
    if (dy > 65 && el && el.scrollTop <= 2) fetchTodos(true);
  ...
  const handleStart = () => {
    ...
    if (timer.isRunning && timer.activeId !== selected.id) {
      const r = timer.stop(); if (r) silentSave(r.todoId, r.totalMin);
    }
  ...
  const handleComplete = async (todoId) => {
    ...
    if (!todo) return;
    const isCurrent = todo.id === selectedId;
    let fin = todo.accum || 0;
    if (isCurrent && isRunning) { const r = timer.stop(); if (r) fin = r.totalMin; }
    else if (isCurrent && isPaused) { fin = paused.savedAccum ?? todo.accum ?? 0; setPaused(null); }

    setTodos(p => p.map(t => t.id === todo.id ? { ...t, done: true, accum: fin } : t));
    if (isCurrent) setSelectedId(null);
  ...
  const handleAddTodo = async (name, date) => {
    ...
    try {
      ...
      setSheet(null);
    } catch (e) { alert(ko ? '저장 실패: ' + e.message : 'Save failed: ' + e.message); }
  };
  ...
  // FAB bottom: above action bar if visible, else above tab bar
  const fabBottom = showBar ? TAB_H + 70 : TAB_H + 16;
  ...
  return (
    ...
    >
      ...
      <div style={{ textAlign: 'center', padding: '52px 24px 16px' }}>
        <div style={{ fontSize: 13, color: 'var(--text3)', fontWeight: 700, marginBottom: 8, letterSpacing: 0.2 }}>
          {fmtDate(locale)}
        </div>
        <div style={{ fontSize: 58, fontWeight: 900, letterSpacing: '-2px', color: 'var(--text)', lineHeight: 1, fontVariantNumeric: 'tabular-nums', marginBottom: 8 }}>
          {fmt(totalMin + (isRunning ? timer.sessionMin : 0))}
        </div>
        {isRunning && (
          <div style={{ fontSize: 15, color: 'var(--text)', fontWeight: 800, fontVariantNumeric: 'tabular-nums', marginBottom: 4, opacity: 0.6, animation: 'pulse 2s ease-in-out infinite' }}>
            ● {timer.formatElapsed()}
          </div>
        )}
        {isPaused && (
        ...
        {todos.length > 0 && (
          <>
            ...
            <div className="prog" style={{ maxWidth: 180, margin: '0 auto' }}>
...
/* ── Bidirectional swipe card ──
   Swipe RIGHT → LEFT action: ✓ complete (green circle)
   Swipe LEFT  → RIGHT actions: edit circle + delete circle
*/
function SwipeCard({ todo, ko, fmt, selected, isRunning, isPaused, liveAccum, liveDisplay, onClick, onComplete, onDelete, delay }) {
  ...
  const tMove  = (e) => {
    ...
    // left: reveal right actions (max -120), right: reveal left action (max +80)
    setSx(Math.min(80, Math.max(-120, dx)));
  };
  const tEnd = () => {
    if (sx < -SNAP) setSx(-110);
    else if (sx > SNAP * 0.7) setSx(70);
    else setSx(0);
    startX.current = null;
  ...
  return (
    <div
      style={{ position: 'relative', borderRadius: 20, overflow: 'hidden', animationDelay: `${delay}ms` }}
      className="slide-in"
    >
      {/* LEFT action: complete (revealed by swipe-right) */}
      <div style={{
        position: 'absolute', left: 0, top: 0, bottom: 0, width: 70,
        display: 'flex', alignItems: 'center', justifyContent: 'center',
      }}>
        <button
          onClick={() => { setSx(0); onComplete(); }}
          style={{
      ...
      {/* RIGHT actions: delete (revealed by swipe-left) */}
      <div style={{
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div
              style={{ fontWeight: 800, fontSize: 15, color: 'var(--text)', textDecoration: todo.done ? 'line-through' : 'none', marginBottom: 2, opacity: todo.done ? 0.5 : 1 }}
              className="truncate"
            >
              {todo.name}
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              {isRunning && liveDisplay && (
                <span style={{ fontSize: 13, fontWeight: 800, color: 'var(--text)', fontVariantNumeric: 'tabular-nums', opacity: 0.6, animation: 'pulse 1.8s ease-in-out infinite' }}>
                  ● {liveDisplay}
                </span>
              )}
          ...
          {/* Chevron */}
          <svg
            width="14" height="14" viewBox="0 0 24 24" fill="var(--text4)"
