...
export async function GET(request) {
  ...
  try {
    ...
    result.steps.push({ step: '2_client_ok' });

    const resp = await notion.databases.query({ database_id: dbTodo, page_size: 3 });
    result.steps.push({ step: '3_query_ok', count: resp.results.length });

    if (resp.results[0]) {
      const propNames = Object.keys(resp.results[0].properties || {});
      result.steps.push({ step: '4_props', names: propNames });
    }
