// app/lib/i18n.js
export const translations = {
  ko: {
    appName: '노크 순공타이머',
    slogan: '집중한 시간이 쌓이는 곳',
    connectNotion: '노션 연결하고 시작',
    signInWithNotion: 'Notion으로 계속',
    enterTokenManually: 'Integration 토큰 직접 입력',
    browse: '둘러보기',
    connectNotionTitle: '노션 연결',
    connectHintChoice: 'Notion으로 로그인하거나, 토큰을 직접 입력할 수 있어요.',
    tokenLabel: 'Integration Token',
    tokenPlaceholder: 'secret_xxxxxxxxxxxx',
    tokenHelp: 'Notion에서 Integration을 만들고 토큰을 입력하세요.',
    howToGetToken: '토큰 발급 방법',
    next: '다음',
    back: '뒤로',
    selectDatabases: 'DB 선택',
    todoDB: 'To-do DB',
    reportDB: 'Daily Report DB',
    selectDB: 'DB를 선택하세요',
    confirmFields: '속성명 확인',
    fieldName: '이름',
    fieldDate: '날짜',
    fieldDone: '완료',
    fieldAccum: '누적(분)',
    fieldReview: '하루 리뷰',
    fieldTotalMin: '집중 합계',
    fieldTodoList: 'To-do List',
    done: '완료',
    finish: '마무리',
    home: '홈',
    log: '기록',
    statsTab: '통계',
    timetableTab: '시간표',
    timetableComingSoon: '시간표 기능은 준비중이에요.',
    settings: '설정',
    totalFocusTime: '오늘 집중시간',
    completionRate: '개 중 {done}개 완료 · {pct}%',
    noTodos: '오늘 할 일이 없어요',
    addFirst: '+ 추가하기',
    start: '시작',
    pause: '멈춤',
    complete: '완료',
    addTodo: '할 일 추가',
    editTodo: '할 일 수정',
    writeFeedback: 'Daily Review',
    todoTitle: '할 일 제목',
    todoTitlePlaceholder: '무엇을 할 건가요?',
    todoWhenLabel: '언제 할거예요',
    todoGoalLabel: '목표',
    goalNone: '선택 안함',
    whenAnytime: '아무때나',
    whenMorning: '오전',
    whenAfternoon: '오후',
    whenEvening: '저녁',
    date: '날짜',
    save: '저장',
    cancel: '취소',
    feedbackPlaceholder: '오늘 공부를 돌아보며 한 줄로 정리해요...',
    graphView: '그래프',
    plannerView: '플래너',
    daily: '일간',
    weekly: '주간',
    monthly: '월간',
    yearly: '연간',
    focusTime: '집중시간',
    focusTimeMinLabel: '포커스 타임(분)',
    noData: '데이터 없음',
    notionLoadingMessage: '노션에서 열심히 자료를 읽어오고 있습니다!!',
    notionOAuthOverlayMessage: 'Notion 로그인 화면으로 연결하는 중…',
    language: '언어',
    weekStart: '주 시작 요일',
    weekStartMonday: '월요일 시작',
    weekStartSunday: '일요일 시작',
    system: '시스템 언어',
    korean: '한국어',
    english: 'English',
    notionConnection: '노션 연결',
    notionManageHint: '계정 · 데이터베이스',
    switchNotionAccount: '다른 Notion 계정으로 연결',
    connected: '연결됨',
    connectedOAuth: 'Notion에 로그인됨',
    notConnected: '연결 안 됨',
    connectNotionCta: '노션 연결하기',
    reconnect: '재연결',
    disconnect: '로그아웃',
    dbProperties: 'DB 속성 설정',
    defaultScreen: '기본 화면',
    missingField: '속성을 찾을 수 없음',
    loadingDbs: 'DB 목록 불러오는 중...',
    errorLoading: '불러오기 실패',
    retry: '다시 시도',
    refresh: '새로고침',
    swipeEdit: '수정',
    swipeDelete: '삭제',
    resetTime: '시간 리셋',
    confirmResetTimeTitle: '집중 시간을 리셋할까요?',
    confirmResetTimeMessage: '"{name}"의 누적 집중 시간이 0으로 초기화됩니다.',
    carryoverTitle: '어제 못다한 일',
    carryoverHint: '이 목록을 오늘 할 일 쪽으로 끌어다 놓거나, 오늘로 눌러 날짜를 오늘로 바꿀 수 있어요.',
    moveToToday: '오늘로',
    dropZoneToday: '여기에 놓으면 오늘 할 일로 옮겨요',
    saved: '저장됨',
    connecting: '연결 중...',
    autoMatched: '자동 매칭됨',
    selectProperty: '속성 선택',
    hours: '시간',
    minutes: '분',
    seconds: '초',
    accumTime: '누적',
    todayDetail: '상세 내역',
    noSession: '기록 없음',
    onboardingStep1: '1 / 4',
    onboardingStep2: '2 / 4',
    onboardingStep3: '3 / 4',
    onboardingStep4: '4 / 4',
    demoMode: '둘러보기 모드',
    connectToSave: '저장하려면 노션을 연결하세요',
    featureComingSoon: '기능 준비중',
  },
  en: {
    appName: 'Nock Study Timer',
    slogan: 'Where focused time accumulates',
    connectNotion: 'Connect Notion & Start',
    signInWithNotion: 'Continue with Notion',
    enterTokenManually: 'Enter integration token',
    browse: 'Browse',
    connectNotionTitle: 'Connect Notion',
    connectHintChoice: 'Sign in with Notion, or enter an integration token below.',
    tokenLabel: 'Integration Token',
    tokenPlaceholder: 'secret_xxxxxxxxxxxx',
    tokenHelp: 'Create a Notion Integration and paste the token here.',
    howToGetToken: 'How to get token',
    next: 'Next',
    back: 'Back',
    selectDatabases: 'Select Databases',
    todoDB: 'To-do DB',
    reportDB: 'Daily Report DB',
    selectDB: 'Select a database',
    confirmFields: 'Confirm Properties',
    fieldName: 'Name',
    fieldDate: 'Date',
    fieldDone: 'Done',
    fieldAccum: 'Focus min',
    fieldReview: 'One-line Review',
    fieldTotalMin: 'Today Focus (min)',
    fieldTodoList: 'To-do List',
    done: 'Done',
    finish: 'Finish',
    home: 'Home',
    log: 'Log',
    statsTab: 'Stats',
    timetableTab: 'Timetable',
    timetableComingSoon: 'Timetable is coming soon.',
    settings: 'Settings',
    totalFocusTime: "Today's Focus",
    completionRate: '{done} of {total} done · {pct}%',
    noTodos: 'No tasks for today',
    addFirst: '+ Add task',
    start: 'Start',
    pause: 'Pause',
    complete: 'Complete',
    addTodo: 'Add Task',
    editTodo: 'Edit Task',
    writeFeedback: 'Daily Review',
    todoTitle: 'Task title',
    todoTitlePlaceholder: 'What will you do?',
    todoWhenLabel: 'When',
    todoGoalLabel: 'Goal',
    goalNone: 'None',
    whenAnytime: 'Anytime',
    whenMorning: 'Morning',
    whenAfternoon: 'Afternoon',
    whenEvening: 'Evening',
    date: 'Date',
    save: 'Save',
    cancel: 'Cancel',
    feedbackPlaceholder: "Reflect on today's study in one line...",
    graphView: 'Graph',
    plannerView: 'Planner',
    daily: 'Daily',
    weekly: 'Weekly',
    monthly: 'Monthly',
    yearly: 'Yearly',
    focusTime: 'Focus Time',
    focusTimeMinLabel: 'Focus (min)',
    noData: 'No data',
    notionLoadingMessage: 'Fetching your data from Notion…',
    notionOAuthOverlayMessage: 'Opening Notion sign-in…',
    language: 'Language',
    weekStart: 'Week starts on',
    weekStartMonday: 'Monday',
    weekStartSunday: 'Sunday',
    system: 'System Language',
    korean: '한국어',
    english: 'English',
    notionConnection: 'Notion Connection',
    notionManageHint: 'Account & databases',
    switchNotionAccount: 'Connect another Notion account',
    connected: 'Connected',
    connectedOAuth: 'Signed in with Notion',
    notConnected: 'Not Connected',
    connectNotionCta: 'Connect to Notion',
    reconnect: 'Reconnect',
    disconnect: 'Log out',
    dbProperties: 'DB Properties',
    defaultScreen: 'Default Screen',
    missingField: 'Property not found',
    loadingDbs: 'Loading databases...',
    errorLoading: 'Failed to load',
    retry: 'Retry',
    refresh: 'Refresh',
    swipeEdit: 'Edit',
    swipeDelete: 'Delete',
    resetTime: 'Reset time',
    confirmResetTimeTitle: 'Reset focus time?',
    confirmResetTimeMessage: 'Accumulated focus time for "{name}" will be reset to 0.',
    carryoverTitle: 'From yesterday',
    carryoverHint: 'Drag into today’s list or tap Move to today to change the date.',
    moveToToday: 'Move to today',
    dropZoneToday: 'Drop here to add to today',
    saved: 'Saved',
    connecting: 'Connecting...',
    autoMatched: 'Auto-matched',
    selectProperty: 'Select property',
    hours: 'h',
    minutes: 'm',
    seconds: 's',
    accumTime: 'Accumulated',
    todayDetail: 'Details',
    noSession: 'No sessions',
    onboardingStep1: '1 / 4',
    onboardingStep2: '2 / 4',
    onboardingStep3: '3 / 4',
    onboardingStep4: '4 / 4',
    demoMode: 'Browse Mode',
    connectToSave: 'Connect Notion to save data',
    featureComingSoon: 'Coming soon',
  },
};

export function getLocale(override) {
  if (override === 'ko') return 'ko';
  if (override === 'en') return 'en';
  if (typeof navigator !== 'undefined') {
    const lang = navigator.language || navigator.userLanguage || 'en';
    return lang.startsWith('ko') ? 'ko' : 'en';
  }
  return 'en';
}

export function useT(locale) {
  const lang = locale || 'en';
  return translations[lang] || translations.en;
}
